...
**2026-09-26 補部署 Firestore（`note_owners`）**：投稿者從便利貼上移到 `note_owners` 的那一版
程式先上了測試站、規則沒跟上，測試站每次送出都被擋（前台顯示成「需要店員 QR Code」）。
部署前確認線上與 repo 的差別只有 `note_owners` 這一段、正式站舊版的匿名條件沒動，
並以 `:test` 跑過 26 項（測試組 11 項全過；正式組有 3 項因【過渡期】的匿名條件而放行，
是已知代價）。仍是步驟 ① 的過渡版，步驟 ②③ 未做。
上一版 ruleset 是 `2d20da99-dc68-4bdd-86bc-a12bddd4d627`，要退回時把 release 指回去即可。
